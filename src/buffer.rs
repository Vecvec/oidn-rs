...
use crate::sys::{
    OIDNBuffer, oidnGetBufferSize, oidnNewBuffer, oidnReadBuffer, oidnReadBufferAsync, oidnReleaseBuffer, oidnWriteBuffer, oidnWriteBufferAsync
};
use std::mem;
use std::sync::Arc;
...
impl Device {
    ...
    pub fn write_buffer_async<'s>(&self, buffer: &'s mut Buffer, contents: &'s [f32]) -> Option<MustSync<'s, ()>> {
        ...
        if buffer.size != contents.len() {
            ...
            Some(MustSync::must_sync(self.0, &mut buffer.sync_lock, Box::new(|| ())))
        }
    }
    ...
    pub fn read_buffer_async<'s>(&self, buffer: &'s mut Buffer) -> MustSync<'s, Vec<f32>> {
        ...
        unsafe {
            oidnReadBufferAsync(buffer.buf, 0, buffer.size * size_of::<f32>(), vec.as_mut_ptr() as *mut _);
        }
        let size  = buffer.size;
        MustSync::must_sync(self.0, &mut buffer.sync_lock, Box::new(move || unsafe {
            let mut vec = vec;
